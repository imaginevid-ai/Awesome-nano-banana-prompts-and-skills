/**
 * [INPUT]: 依赖 twitterapi.io 候选字段、本地既有提示词指纹与 Nano Banana 明确模型证据
...
export function scoreStoredTwitterPrompt(prompt: StoredTwitterPrompt): QualityResult {
  ...
  if (prompt.sourceMeta?.model_evidence !== "nano-banana") {
    hardFailures.push("missing verified Nano Banana model evidence");
  }
  ...
  if (prompt.imageCategories?.workflows?.length !== 1) {
  ...
  if (prompt.imageCategories?.workflows?.length === 1) score += 5;
...
export function scoreTwitterCandidate(candidate: TwitterCandidate): QualityResult {
  ...
  const hasNanoBananaEvidence = /\bnano[-\s]*banana\b|\bgemini[-\s]*2\.5[-\s]*flash[-\s]*image\b/i.test(
    relevanceText
  );
  const namesDifferentFamilyModel = /\bnano[-\s]*banana\s*(?:2(?:\s*lite)?|lite|pro)\b/i.test(
    relevanceText
  );
  if (hasPromptSignal) {
    score += 30;
  ...
  if (!hasNanoBananaEvidence) {
    reasons.push("missing explicit Nano Banana evidence; manual review required");
  ...
  if (namesDifferentFamilyModel) {
    reasons.push("explicitly names a different Nano Banana family model");
    decision = "reject";
  }
